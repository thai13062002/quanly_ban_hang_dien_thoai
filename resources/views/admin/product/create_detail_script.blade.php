   <script type="text/javascript">
       $(document).ready(function() {
           ...
           $(document).on('click', '.list-color', function() {
               ...
               var url = '{{ route('admin.product.list_color_product', ':id') }}';
           ...
           $(document).on('click', '.delete-color', function() {
               ...
               let obj = $(this);
               $.ajax({
                   url: "{{ route('admin.product.delete_color_product') }}",
                   type: 'DELETE',
                   data: {
                       "_token": "{{ csrf_token() }}",
           ...
           $(document).on('click', '.update-color', function() {
               ...
               let obj = $(this)
               $.ajax({
                   url: "{{ route('admin.product.update_color_product') }}",
                   type: 'PUT',
                   data: {
                       "_token": "{{ csrf_token() }}",
                       product_id: product_id,
                   ...
                   success: function(result) {
                       console.log($('.quantity' + product_id), result['data'])
                       obj.attr('disabled', true)
                       $('.quantity-' + product_id).text(result['data'])
                       Swal.fire({
                           icon: 'success',
           ...
           $(document).on('click', '.update-detail', function() {
               ...
               let id = $(this).attr('data-id')
               var url = '{{ route('admin.product.product_detail', ':id') }}';
               url = url.replace(':id', id);
               console.log(url)
               $.ajax({
                   url: url,
           ...
           function removeImg(id, obj) {
               ...
               $.ajax({
                   url: "{{ route('admin.product.detele_img_product_detai') }}",
                   type: 'DELETE',
                   data: {
                       "_token": "{{ csrf_token() }}",
                       id: id,
           ...
           $(document).on('click', '.remove', function() {
               ...
               $.ajax({
                   url: "{{ route('admin.product.detele_product_detail') }}",
                   type: 'DELETE',
                   data: {
                       "_token": "{{ csrf_token() }}",
                       id: id
